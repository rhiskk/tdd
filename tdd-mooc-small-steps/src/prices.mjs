...
function createApp(database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const cost = calculateCost(age, type, date, temporalDate, baseCost);
    res.json({ cost });
  });
  ...
  function calculateCost(age, type, date, temporalDate, baseCost) {
    if (type === "night") {
  ...
  function calculateReduction(date, temporalDate) {
    let reduction = 0;
    if (date && temporalDate && isMonday(date, temporalDate) && !isHoliday(date, temporalDate)) {
      reduction = 35;
    }
    return reduction;
  }
